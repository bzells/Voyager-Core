...
public class VoyagerItems {
    ...
    static {
        for (int tier : MODULE_TIERS) {
            createHelperModifierModule("basic_helper_module", "Basic Helper Module", tier, 0,
                    .15f / ((float) tier * .5f), .05f + (((float) tier / 8) * .3f), 0, false, 1);
            createHelperModifierModule("efficiency_helper_module", "Efficiency Helper Module", tier, 0,
                    1.2f / (float) tier, -.4f + (.5f * ((float) tier / 8)), 0, false, 1);
            createHelperModifierModule("speed_helper_module", "Speed Helper Module", tier, 0,
                    -0.25f * ((float) tier) / 4, .2f + (.4f * ((float) tier / 6)), 0, false, 1);
            createHelperModifierModule("output_helper_module", "Output Modifier Helper Module", tier, 0,
                    0.05f * ((float) tier) / 4, -.2f + (.4f * ((float) tier / 8)), .5f, true, 2);
            createHelperModifierModule("parallel_helper_module", "Parallel Modifier Helper Module", tier, tier,
                    0.05f * ((float) tier) / 4, -.8f + (.1f * ((float) tier / 8)), 0f, false, 2);
            ...
            ItemEntry<Item> HELPER_MODULE_BASE = VOYAGERCORE_REGISTRATE
                    ...
                    .lang(VoyagerVoltageTierUtils.getVoltageTierColorStringShortForm(GTValues.VN[tier]) +
                            " Helper Module Base")
                    .register();
        }
